...
setMethod(
  ...
  definition = function(theObject, grouping_variable, title = "", font_size = 8) {
    ...
    # Call plotDensity on the ggplot object
    plotDensity(pca_plot, grouping_variable, title, font_size)
  }
)
...
setMethod(
  ...
  definition = function(theObject, pca_titles = NULL, density_titles = NULL, rle_titles = NULL, pearson_titles = NULL, save_path = NULL, file_name = "pca_density_rle_pearson_corr_plots_merged") {
    ...
    getPcaMinMax <- function(theObject) {
      pca_xmax <- purrr::map_dbl(theObject@pca_plots, \(x){
        max(x$data$PC1)
      }) |> max()
      pca_xmin <- purrr::map_dbl(theObject@pca_plots, \(x){
        min(x$data$PC1)
      }) |> min()
      pca_ymax <- purrr::map_dbl(theObject@pca_plots, \(x){
        max(x$data$PC2)
      }) |> max()
      pca_ymin <- purrr::map_dbl(theObject@pca_plots, \(x){
        min(x$data$PC2)
      }) |> min()
    ...
    created_rle_plots <- lapply(theObject@rle_plots, \(x){
      createRlePlot(x, rle_min_max$ymin, rle_min_max$ymax)
    })
